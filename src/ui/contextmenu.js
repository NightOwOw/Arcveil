...
import { pickNodeImage } from '../canvas/node-image.js';
...
export function showNodeMenu(id, x, y) {
  ...
  const items = [
    ...
    'sep',
    { icon:'⊕', label:'Replace Avatar', action: async () => {
      await pickNodeImage(id);
    }},
...
export function showEdgeMenu(id, x, y) {
  ...
  _show(x, y, [
    ...
    { icon:'≋', label:'Style: ' + (edge.style||'solid'), action: () => {
...
export function showMultiSelectMenu(x, y, ids) {
  _show(x, y, [
    { icon:'○', label:`${ids.length} nodes selected`, action: () => {} },
    'sep',
    { icon:'×', label:`Delete ${ids.length} nodes`, danger: true, action: () => {
